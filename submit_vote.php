	...
	include 'dbconnector/dbcon.php';
		...
		try {
				...
				session_destroy();
						...
						foreach($_SESSION['p_id'] as $row =>$value){
						
			
						$conn->query("INSERT INTO `votes` VALUES('', '$value',   '$_SESSION[voters_id]')") or die($conn->error);
						}
						...
						$conn->query("UPDATE `voters` SET `status` = 'Voted' WHERE `voters_id` = '$_SESSION[voters_id]'") or die($conn->error);
				            ...
				            $peace = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[p_id]'")->fetch_array();
				            $pcfirstname = $peace['firstname'];
				            $pclastname = $peace['lastname'];
				            $pcdepartment = $peace['department'];

				             $acor = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[a_id]'")->fetch_array();
				            $acfirstname = $acor['firstname'];
				            $aclastname = $acor['lastname'];
				            $acdepartment = $acor['department'];
							
							$acor = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[st]'")->fetch_array();
				            $acfirstname = $acor['firstname'];
				            $aclastname = $acor['lastname'];
				            $acdepartment = $acor['department']; 

							$acor = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[nd]'")->fetch_array();
				            $acfirstname = $acor['firstname'];
				            $aclastname = $acor['lastname'];
				            $acdepartment = $acor['department']; 

							$acor = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[rd]'")->fetch_array();
				            $acfirstname = $acor['firstname'];
				            $aclastname = $acor['lastname'];
				            $acdepartment = $acor['department']; 

							$acor = $conn->query("SELECT * FROM tbl_candidate WHERE candidate_id = '$_SESSION[th]'")->fetch_array();
				            $acfirstname = $acor['firstname'];
				            $aclastname = $acor['lastname'];
				            $acdepartment = $acor['department']; 
			// start comment	            
